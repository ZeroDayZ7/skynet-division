...
export const getUserDetailsForValidation = async (email: string): Promise<UserAttributes | null> => {
  const user = await User.findOne({
    where: { email },
    attributes: ['id', 'points', 'pass', 'activation_token', 'userBlock', 'role', 'loginAttempts', 'lastLoginAttempt', 'documents', 'username'],
  });
...
export const checkEmailAvailability = async (email: string): Promise<boolean> => {
  ...
  return !user; // Zwraca true, jeśli email jest dostępny, false, jeśli zajęty
};
