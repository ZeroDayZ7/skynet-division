...
import UserEIDData from "#ro/auth/models/UserEIDData";
import UserData from "#ro/auth/models/UserData";
...
export const fetchUserEIDData: (userId: number) => Promise<UserEIDDataResponse | null> = async (userId) => {
  try {
    ...
    SystemLog.error("Błąd pobierania danych e-dowodu w serwisie", { userId, error });
